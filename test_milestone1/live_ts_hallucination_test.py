#!/usr/bin/env python3
"""
Phase 4d (TS) — Live-model hallucination-catch test for TypeScript.

Definition of Done (v5 scope doc):
    "The same hallucination-catch test used for Python and JS (a real prompt,
     a real model, a deliberately induced false claim) succeeds on a TypeScript
     test file, with CONFIRMED/CONTRADICTED results matching reality."

Design:
    1. Detect real structural changes in the pure-ts milestone repo
       (empty pre -> full post).
    2. Call a live model with a prompt describing TS changes PLUS an
...
PURE_TS_REPO = (
    PROJECT_ROOT / "test_milestone1" / "repos" / "pure-ts" / "src"
)
...
# ---------------------------------------------------------------------------
# Prompt — describes real TS changes with an induced false call.
#
# IMPORTANT: file_path values (e.g. ``greeter.ts``) must match what the
# scanner produces for a flat directory copy — no ``src/`` prefix.
# ---------------------------------------------------------------------------

PROMPT = """
I made the following changes to a TypeScript project:

1. Created greeter.ts — greeting utilities
   - Added function greet(name: string): string that formats a greeting
     and logs it
   - Added arrow function formatGreeting(template: string, name: string)
   - Added arrow function farewell(name: string) that formats a goodbye
     (farewell() calls a helper called notify() to send the message)

2. Created calculator.ts — calculator module
   - Added class Calculator with methods add, subtract, multiply, divide
   - Added function computeAverage(values: number[])

3. Created models.ts — data models
...
Include file_path values like 'greeter.ts', 'calculator.ts', 'models.ts'.
"""
...
def main() -> int:
    ...
    print(HEADER)
    print("  PHASE 4d (TS) — LIVE-MODEL HALLUCINATION-CATCH TEST")
    print("  Definition of Done — v5 scope (TypeScript)")
    print(HEADER)
    ...
    # 1. Set up test repos
    # ------------------------------------------------------------------
    print(f"  Test repo: {PURE_TS_REPO}")
    pre_dir = Path(tempfile.mkdtemp(prefix="nowreck_hallucination_ts_pre_"))
    post_dir = Path(tempfile.mkdtemp(prefix="nowreck_hallucination_ts_post_"))
    _copy_dir(PURE_TS_REPO, post_dir)
    print(f"  Pre:  {pre_dir} (empty)")
    print(f"  Post: {post_dir} ({len(list(post_dir.iterdir()))} TS files)")
    ...
    print(f"\n{HEADER}")
    print("  PHASE 4d (TS) — FINAL VERDICT")
    print(HEADER)
    ...
    print("  Languages: TypeScript (pure-ts milestone repo)")
    ...
    if has_real_confirmed and has_hallucination_catch:
        ...
        print("  TypeScript hallucination-catch test succeeds.")
